...
+ (CGSize)setLabelSize:(NSString *)text font:(CGFloat)font
{
   ...
   return [text sizeWithAttributes:dic];
}
//根据文本设置label的尺寸（多行）
...
+ (NSMutableAttributedString *)setLabelText:(NSString *)string1 string2:(NSString *)string2 string1Font:(CGFloat)string1Font string2Font:(CGFloat)string2Font color:(UIColor *)color
{
    ...
    NSRange str1 = NSMakeRange(0,string1.length);
    NSRange str2 = NSMakeRange(string1.length + 2,string2.length);
    [attrString addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:string1Font],NSForegroundColorAttributeName:color} range:str1];
    [attrString addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:string2Font],NSForegroundColorAttributeName:color} range:str2];
    return attrString;
}
...
+ (CGFloat)getStingHeight:(NSString *)string
{
    ...
    lable.text=string;
    lable.font=[UIFont systemFontOfSize:16];
    CGSize size=[lable boundingRectWithSize:CGSizeMake(kDeviceWidth-20, 0)];
    return size.height;
...
@implementation NSData (ASEEncryption)
